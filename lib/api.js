...
export async function getWeatherData() {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?zip=13244&appid=${openWeatherApiKey}&units=metric`
    );
...
export const getGeoLocation= () => {
  return new Promise ((resolve, reject) =>{
    //if pizza are avaiable
    if ("geolocation" in navigator) {
      //resolve();
      // two function are required to get the location
      ...
      //if no pizzas are avaiable
      reject("Geolocation is not avaiable");
    }
  });
};
// export const getGeoLocation= () => {
//   // state: pending, resolved, rejected
//   // two arguments: reolve, reject
//   return new Promise((resolve, reject)=>{
//     //if pizza is ready
//     if ("geolocation" in navigator) {
//       //reolve ();
//       // twofunctions are needed inside: success, error
//       navigator.geolocation.getCurrentPosition(
//         (position) => {
//           resolve(position);
//         },
